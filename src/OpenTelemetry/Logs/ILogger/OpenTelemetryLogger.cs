...
internal sealed class OpenTelemetryLogger : ILogger
{
        ...
        bool parseStateValues)
    {
        ...
        {
            return ParseCustomState(logRecord, in state);
        }
        else
        {
            OpenTelemetrySdkEventSource.Log.LoggerStateParsingSkipped<TState>();
    ...
    [UnconditionalSuppressMessage("ReflectionAnalysis", "IL2026", Justification = "Only called when RuntimeFeature.IsDynamicCodeSupported")]
    private static IReadOnlyList<KeyValuePair<string, object?>> ParseCustomState<TState>(LogRecord logRecord, in TState state)
    {
        try
        {
